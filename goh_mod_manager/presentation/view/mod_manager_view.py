...
class ModManagerView(QMainWindow):
    ...
    def retranslate(self) -> None:
        ...
        self.ui.label_reorder_tip.setText(
            self.tr("Tip: Drag and drop to reorder active mods.")
        )
    ...
    def clear_mod_lists(self) -> None:
        ...
        self.ui.listWidget_installed_mods.clear()
    ...
    def get_current_available_mod(self) -> List[QListWidgetItem]:
        ...
        return self.ui.listWidget_installed_mods.selectedItems()

    def get_selected_available_mod_ids(self) -> List[str]:
        """Get IDs of currently selected available mods."""
        selected_ids = []
        for item in self.ui.listWidget_installed_mods.selectedItems():
            mod = item.data(Qt.UserRole)
            if mod:
    ...
    def clear_selections(self) -> None:
        ...
        self.ui.listWidget_installed_mods.clearSelection()
    ...
    def update_active_mods_count(self, count: int) -> None:
        ...
        """
        self.ui.lcdNumber_active_count.display(count)
    ...
    def update_mod_details(self, mod: Optional[Mod]) -> None:
        ...
        if mod:
            ...
            self.ui.label_mod_title.setTextFormat(Qt.RichText)
            formatted_name = self.parse_formatted_text(mod.name)
            self.ui.label_mod_title.setText(f"{formatted_name} [{mod.id}]")
            ...
            self.ui.label_mod_title.setTextFormat(Qt.PlainText)
            self.ui.label_mod_title.setText(self.tr("No mod selected"))
            self.ui.textEdit_mod_description.clear()

    def highlight_required_mods(self, required_ids: set[str]) -> None:
        palette = self.ui.listWidget_installed_mods.palette()
        default_text_color = palette.color(QPalette.Text)

        for i in range(self.ui.listWidget_installed_mods.count()):
            item = self.ui.listWidget_installed_mods.item(i)
            item_mod = item.data(Qt.UserRole)

            if item_mod and item_mod.id in required_ids:
                item.setForeground(QBrush(QColor("#E67E22")))
            else:
                item.setForeground(QBrush(default_text_color))

    def reset_mod_colors(self) -> None:
        palette = self.ui.listWidget_installed_mods.palette()
        default_text_color = palette.color(QPalette.Text)
    ...
    def set_busy(self, busy: bool) -> None:
        """Disable interactions that should not run concurrently."""
        self.ui.pushButton_refresh_installed_mods.setEnabled(not busy)
        self.ui.actionRefresh.setEnabled(not busy)
        self.ui.action_local_mod.setEnabled(not busy)
        self.ui.action_load_order_from_code.setEnabled(not busy)
        self.ui.action_load_order_as_code.setEnabled(not busy)
    ...
    def _add_mod_to_available(self, mod: Mod) -> None:
        ...
        self._add_mod_to_list(
            self.ui.listWidget_installed_mods, mod, bold=mod.manualInstall
        )
